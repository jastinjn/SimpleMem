...
class TestMemoryStats:
    async def test_missing_user_id_is_422(self, client):
        assert (await client.post("/memory/stats", json={})).status_code == 422

    async def test_empty_user_id_is_422(self, client):
        assert (await client.post("/memory/stats", json={"user_id": ""})).status_code == 422

    async def test_entry_count_matches_corpus(self, client):
        body = (await client.post("/memory/stats", json={"user_id": USER_ID, "scope_id": SCOPE})).json()
    ...
    async def test_empty_scope_returns_zeros(self, client):
        body = (await client.post("/memory/stats", json={"user_id": USER_ID, "scope_id": OTHER_SCOPE})).json()
        assert body["entry_count"] == 0
        assert body["total"] == 0
        assert body["superseded"] == 0

    async def test_no_scope_returns_all_user_memories(self, client):
        body = (await client.post("/memory/stats", json={"user_id": USER_ID})).json()
        assert body["entry_count"] == CORPUS_SIZE
