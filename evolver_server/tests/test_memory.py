...
class TestMemoryAdd:
    ...
    def test_empty_scope_id_is_422(self, client):
        assert client.post("/memory/add", json={"scope_id": "", "prompt_text": "hi"}).status_code == 422

    def test_both_fields_empty_is_422(self, client):
        assert client.post("/memory/add", json={"scope_id": SCOPE}).status_code == 422
    ...
    def test_consolidates_after_add(self, client):
        ...
        s1 = client.post("/memory/stats", json={"scope_id": SCOPE}).json()
        ...
        s2 = client.post("/memory/stats", json={"scope_id": SCOPE}).json()
    ...
    def test_does_not_write_to_other_scope(self, client):
        ...
        assert client.post("/memory/stats", json={"scope_id": SCOPE}).json()["entry_count"] == CORPUS_SIZE
        r = client.post("/memory/retrieve", json={"scope_id": SCOPE, "query": "database", "top_k": 10})
...
class TestMemoryAddBatch:
    ...
    def test_empty_turns_is_422(self, client):
        assert client.post("/memory/add_batch", json={"scope_id": SCOPE, "turns": []}).status_code == 422
    ...
    def test_new_content_is_retrievable(self, client):
        ...
        r = client.post("/memory/retrieve", json={"scope_id": SCOPE, "query": "Helm kubernetes", "top_k": 10})
        assert r.status_code == 200
        assert r.json()["total"] > 0
...
class TestMemoryRetrieve:
    ...
    def test_missing_query_is_422(self, client):
        assert client.post("/memory/retrieve", json={"scope_id": SCOPE}).status_code == 422
    ...
    def test_top_k_limits_result_count(self, client):
        r = client.post("/memory/retrieve", json={"scope_id": SCOPE, "query": "database", "top_k": 1})
        assert len(r.json()["results"]) <= 1

    def test_returns_empty_for_unrelated_query(self, client):
        r = client.post("/memory/retrieve", json={"scope_id": SCOPE, "query": "xyzzy_zap_unrelated_42", "top_k": 10})
        assert r.json()["total"] == 0

    def test_returns_empty_for_unknown_scope(self, client):
        r = client.post("/memory/retrieve", json={"scope_id": OTHER_SCOPE, "query": "database", "top_k": 10})
        assert r.json()["total"] == 0
...
class TestMemoryClear:
    ...
    def test_clears_existing_memories(self, client):
        r = client.post("/memory/clear", json={"scope_id": SCOPE})
        assert r.status_code == 200
        body = r.json()
        assert body["scope_id"] == SCOPE
        ...
        assert client.post("/memory/stats", json={"scope_id": SCOPE}).json()["entry_count"] == 0
        assert client.post("/memory/retrieve", json={"scope_id": SCOPE, "query": "database", "top_k": 10}).json()["total"] == 0

    def test_empty_scope_returns_zero_archived(self, client):
        r = client.post("/memory/clear", json={"scope_id": OTHER_SCOPE})
        assert r.status_code == 200
    ...
    def test_does_not_affect_other_scope(self, client):
        ...
        bob_before = client.post("/memory/stats", json={"scope_id": OTHER_SCOPE}).json()["entry_count"]
        client.post("/memory/clear", json={"scope_id": SCOPE})
        bob_after = client.post("/memory/stats", json={"scope_id": OTHER_SCOPE}).json()["entry_count"]
