...
class TestMemoryAdd:
    async def test_missing_user_id_is_422(self, client):
        assert (await client.post("/api/memory/add", json={"prompt_text": "hi"})).status_code == 422

    async def test_empty_user_id_is_422(self, client):
        assert (await client.post("/api/memory/add", json={"user_id": "", "prompt_text": "hi"})).status_code == 422

    async def test_both_fields_empty_is_422(self, client):
        assert (await client.post("/api/memory/add", json={"user_id": USER_ID})).status_code == 422
    ...
    async def test_new_content_is_retrievable(self, client_and_store):
        ...
        r = await client.post("/api/memory/retrieve", json={
            "user_id": USER_ID,
            "scope_id": SCOPE,
    ...
    async def test_consolidates_after_add(self, client):
        ...
        r1 = await client.post("/api/memory/add", json={
            "user_id": USER_ID,
            "scope_id": SCOPE,
    ...
    async def test_does_not_write_to_other_scope(self, client):
        ...
        assert (await client.post("/api/memory/stats", json={"user_id": USER_ID, "scope_id": SCOPE})).json()["entry_count"] == CORPUS_SIZE
        r = await client.post("/api/memory/retrieve", json={"user_id": USER_ID, "scope_id": SCOPE, "query": "database", "top_k": 10})
    ...
    async def test_does_not_write_to_other_user(self, client):
        ...
        assert (await client.post("/api/memory/stats", json={"user_id": USER_ID, "scope_id": SCOPE})).json()["entry_count"] == CORPUS_SIZE
        assert (await client.post("/api/memory/stats", json={"user_id": USER_ID})).json()["entry_count"] == CORPUS_SIZE


class TestMemoryAddBatch:
    async def test_missing_user_id_is_422(self, client):
        assert (await client.post("/api/memory/add_batch", json={"turns": [{"prompt_text": "hi"}]})).status_code == 422

    async def test_empty_turns_is_422(self, client):
        assert (await client.post("/api/memory/add_batch", json={"user_id": USER_ID, "scope_id": SCOPE, "turns": []})).status_code == 422

    async def test_over_50_turns_is_422(self, client):
        turns = [{"prompt_text": f"Turn {i}", "response_text": "Ok."} for i in range(51)]
        assert (await client.post("/api/memory/add_batch", json={"user_id": USER_ID, "scope_id": SCOPE, "turns": turns})).status_code == 422

    async def test_returns_units_added(self, client):
        r = await client.post("/api/memory/add_batch", json={
            "user_id": USER_ID,
            "scope_id": SCOPE,
            "turns": [
    ...
    async def test_new_content_is_retrievable(self, client):
        await client.post("/api/memory/add_batch", json={
            "user_id": USER_ID,
            "scope_id": SCOPE,
            "turns": [
                {"prompt_text": "We use Helm for Kubernetes package management", "response_text": "Got it."},
        ...
        r = await client.post("/api/memory/retrieve", json={"user_id": USER_ID, "scope_id": SCOPE, "query": "Helm kubernetes", "top_k": 10})
        assert r.status_code == 200
        assert r.json()["total"] > 0
...
class TestMemoryRetrieve:
    async def test_missing_user_id_is_422(self, client):
        assert (await client.post("/api/memory/retrieve", json={"query": "x"})).status_code == 422

    async def test_missing_query_is_422(self, client):
        assert (await client.post("/api/memory/retrieve", json={"user_id": USER_ID, "scope_id": SCOPE})).status_code == 422
    ...
    async def test_top_k_limits_result_count(self, client):
        r = await client.post("/api/memory/retrieve", json={"user_id": USER_ID, "scope_id": SCOPE, "query": "database", "top_k": 1})
        assert len(r.json()["results"]) <= 1

    async def test_returns_empty_for_unrelated_query(self, client):
        r = await client.post("/api/memory/retrieve", json={"user_id": USER_ID, "scope_id": SCOPE, "query": "xyzzy_zap_unrelated_42", "top_k": 10})
        assert r.json()["total"] == 0

    async def test_returns_empty_for_unknown_scope(self, client):
        r = await client.post("/api/memory/retrieve", json={"user_id": USER_ID, "scope_id": OTHER_SCOPE, "query": "database", "top_k": 10})
        assert r.json()["total"] == 0
    ...
    async def test_scope_filters_out_other_scopes(self, client):
        ...
        r = await client.post("/api/memory/retrieve", json={"user_id": USER_ID, "scope_id": SCOPE, "query": "Terraform", "top_k": 10})
        assert r.json()["total"] == 0
    ...
    async def test_no_scope_returns_all_user_scopes(self, client):
        ...
        r = await client.post("/api/memory/retrieve", json={"user_id": USER_ID, "query": "Terraform", "top_k": 10})
        assert r.json()["total"] > 0
    ...
    async def test_user_cannot_access_other_user_memories_in_same_scope(self, client):
        ...
        r = await client.post("/api/memory/retrieve", json={"user_id": USER_ID, "scope_id": SCOPE, "query": "Ansible", "top_k": 10})
        assert r.json()["total"] == 0
        r = await client.post("/api/memory/retrieve", json={"user_id": USER_ID, "query": "Ansible", "top_k": 10})
        assert r.json()["total"] == 0


class TestMemoryClear:
    async def test_missing_user_id_is_422(self, client):
        assert (await client.post("/api/memory/clear", json={})).status_code == 422

    async def test_clears_existing_memories(self, client):
        r = await client.post("/api/memory/clear", json={"user_id": USER_ID, "scope_id": SCOPE})
        assert r.status_code == 200
        body = r.json()
        assert body["user_id"] == USER_ID
        assert body["scope_id"] == SCOPE
        ...
        assert (await client.post("/api/memory/stats", json={"user_id": USER_ID, "scope_id": SCOPE})).json()["entry_count"] == 0
        assert (await client.post("/api/memory/retrieve", json={"user_id": USER_ID, "scope_id": SCOPE, "query": "database", "top_k": 10})).json()["total"] == 0

    async def test_empty_scope_returns_zero_archived(self, client):
        r = await client.post("/api/memory/clear", json={"user_id": USER_ID, "scope_id": OTHER_SCOPE})
        assert r.status_code == 200
    ...
    async def test_does_not_affect_other_scope(self, client):
        ...
        bob_before = (await client.post("/api/memory/stats", json={"user_id": USER_ID, "scope_id": OTHER_SCOPE})).json()["entry_count"]
        await client.post("/api/memory/clear", json={"user_id": USER_ID, "scope_id": SCOPE})
        bob_after = (await client.post("/api/memory/stats", json={"user_id": USER_ID, "scope_id": OTHER_SCOPE})).json()["entry_count"]
    ...
    async def test_does_not_affect_other_user_same_scope(self, client):
        ...
        other_before = (await client.post("/api/memory/stats", json={"user_id": other_user, "scope_id": SCOPE})).json()["entry_count"]
        await client.post("/api/memory/clear", json={"user_id": USER_ID, "scope_id": SCOPE})
        other_after = (await client.post("/api/memory/stats", json={"user_id": other_user, "scope_id": SCOPE})).json()["entry_count"]
