...
class TestMemoryUnit:
    ...
    def test_list_fields_are_independent(self):
        u1 = MemoryUnit(
            memory_id="m1", scope_id="s", memory_type=MemoryType.SEMANTIC,
            content="a", created_at="2025-01-01T00:00:00+00:00",
            updated_at="2025-01-01T00:00:00+00:00",
        )
        u2 = MemoryUnit(
            memory_id="m2", scope_id="s", memory_type=MemoryType.SEMANTIC,
    ...
    def test_explicit_timestamps_preserved(self):
        u = MemoryUnit(
            memory_id="m1", scope_id="s", memory_type=MemoryType.EPISODIC,
            content="c",
...
class TestMemoryQuery:
    def test_defaults(self):
        q = MemoryQuery(scope_id="s", query_text="hello")
        assert q.top_k == 6
    ...
    def test_custom_values(self):
        q = MemoryQuery(scope_id="s", query_text="x", top_k=3, max_tokens=200)
        assert q.top_k == 3
